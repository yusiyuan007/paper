...
\begin{abstract}
  随着片上系统的设计复杂性不断提高，在设计前期通过对系统层次进行软、硬件的划分
  对片上系统各方面性能的影响日趋增加，迫切需要高效快捷的性能分析和验证方法学。
  而ESL(Electronic System Level)设计方法不仅提供快速的仿真验证方法，还提供了
  详细的性能分析指标，现如今已经成为SoC(System on Chip)设计领域最先进的设计方
  法。

  在芯片设计过程中，我们需要不断的对系统的设计参数进行修改从而得到性能更优的芯片架构。
  但由于片上系统设计复杂性的提高，片上系统的系统参数变得更多而且范围更广，去考虑系统设计的每个方面变得不那么可能，因此我们需要自动化的工
  具或者流程去完成前期对巨大的设计空间的剪枝，设计空间探索（Design Space 
  Exploration，DSE）成为了一种很好的方法去实现这一功能。但在如今系统级仿真过程
  中，仿真精度以及仿真的严谨性的要求下，仿真系统内部的复杂度越来越大，从而导致
  现如今的仿真系统不适于进行快速的架构探索，所以我们需要建立一个轻量级的仿真平
  台，并在这个平台的基础上进行快速的架构探索。针对以上问题，本文主要完成了以下
  几部分工作：
